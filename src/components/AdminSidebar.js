...
const AdminSidebar = () => {
  ...
  return (
    <aside
      className={`${
        !isCollapsed ? '-left-[250px] md:hidden' : 'left-0 md:block'
      } absolute md:relative transition-all duration-300 select-none z-20 flex-shrink-0 w-[250px] overflow-y-auto bg-[#081d54] h-screen`}
    >
        ...
        {menuItems.map((v, i) => {
          return (
            <Link
              ...
              className={`${
                '' === v.path ? 'bg-[#4e6aaf]' : ''
              } p-2 mt-3 flex items-center rounded-md cursor-pointer hover:bg-[#4e6aaf] text-white`}
              to={v.path}
            >
              {v.icon}
